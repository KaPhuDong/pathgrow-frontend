...
const UserTable = () => {
    ...
    const handleAddOrUpdateUser = async (userData) => {
      try {
        if(userData.id) {
          //Update user
          const response = await axios.put(`http://127.0.0.1:8000/api/admin/users/${userData.id}`, userData);
          if (response.data) {
            setUsers(prev =>
          ...
          const response = await axios.post(`http://127.0.0.1:8000/api/admin/users`, userData);
          if (response.data) {
  ...
  return (
    <div className="user-management container py-4">
        <div className="user-management__header">
          ...
          <button className="btn-logout">Log out 
      ...
      <div className="user-management__controls mb-3">
        <div className="dropdown">
            ...
            <ul className="dropdown-menu" aria-labelledby="userTypeDropdown">
                <li><button className="dropdown-item" onClick={() => setFilterRole("all")}>All users</button></li>
                <li><button className="dropdown-item" onClick={() => setFilterRole("teacher")}>All teachers</button></li>
                <li><button className="dropdown-item" onClick={() => setFilterRole("student")}>All students</button></li>
            </ul>
        </div>

        <button className="btn-add-user text-white" onClick={() => setShowAddUserModal(true)}>
        ...
        <table className="table table-bordered text-center align-middle">
          <thead className="table-success">
            <tr>
              ...
              <th>Email</th>
              <th>Password</th>
              <th className="th-action" style={{width:'120px'}}>Action</th>
            </tr>
          </thead>
          <tbody>
