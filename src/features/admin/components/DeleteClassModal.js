...
const DeleteClassModal = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      if (tab === 'subjects') {
        await axios.post(
          `https://pathgrow-backend-z6tf.onrender.com/api/classesManagement/${classId}/remove-subjects`,
          { subjects: selectedIds }
        );
      } else if (tab === 'students') {
        await axios.post(
          `https://pathgrow-backend-z6tf.onrender.com/api/classesManagement/remove-students`,
          { students: selectedIds }
        );
      } else if (tab === 'teachers') {
        await axios.post(
          `https://pathgrow-backend-z6tf.onrender.com/api/classesManagement/remove-teachers`,
